...
@torch.no_grad()
def epoch_test(dataloader, model, device, text_test_embed, args):
    model.eval() 
    logit_scale = model.logit_scale.detach()
    start_time = time.time()  


    txt_embed = model.text_projection(text_test_embed.float().to(device)) 
    text_embeds = txt_embed / txt_embed.norm(dim=1, keepdim=True) 

    image_embeds = []
    for image, img_id in dataloader: 
        image_feat = model.image_encoder(torch.stack(image).to(device))
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, text_test_embed, return_loss=False):
    ...
    for ep in tqdm(range(Epoch+1), ncols=60):
        ...
        if ep in eval_epochs:
            with torch.no_grad():
                score_val_i2t, score_val_t2i = epoch_test(testloader, net, args.device, text_test_embed)
...
class SimilarityDataloader:
    ...
    def __iter__(self):
        ...
        for _ in range(num_batch):
            ...
            if self.similarity_train is None:
                yield self.images_train[ids], self.labels_train[ids], None
            else:
...
def evaluate_synset_with_similarity(it_eval, net, images_train, labels_train, lr_img, lr_txt, similarity_train, testloader, args, text_test_embed, mom=0.9, l2=0.0005):
    ...
    images_train = images_train.to(args.device)
    labels_train = labels_train.to(args.device)
    if similarity_train is not None:
    ...
    eval_freq = Epoch // 10
    eval_epochs = list(range(0, Epoch+1, eval_freq))  # eval 10 times in total

    best_val_result = None
    for ep in tqdm(range(Epoch+1), ncols=60, disable=not sys.stdout.isatty()):
        loss_train, acc_train = epoch(ep, trainloader, net, optimizer, args)
        acc_train_list.append(acc_train)
        loss_train_list.append(loss_train)
        if ep in eval_epochs:
            with torch.no_grad():
                score_val_i2t, score_val_t2i = epoch_test(testloader, net, args.device, text_test_embed, args)
                val_result = itm_eval(score_val_i2t, score_val_t2i, testloader.dataset.txt2img, testloader.dataset.img2txt)
