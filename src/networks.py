...
@functools.lru_cache(maxsize=128)
def get_gpt1_stuff():
    model_name = 'data/checkpoints/openai-gpt'
...
# @functools.lru_cache(maxsize=128)
def load_from_timm(model_name, pretrained, args):
    
    preprocess=None 
    
    if model_name == 'clip':
        model = CLIPModel.from_pretrained(args.clip_model_id).to(args.device)
        preprocess = CLIPProcessor.from_pretrained(args.clip_model_id) 

    elif model_name == 'nfnet':
        model = timm.create_model('nfnet_l0', pretrained=pretrained, num_classes=0, global_pool="avg",
                                        pretrained_cfg_overlay=dict(file='data/checkpoints/nfnet_l0_ra2-45c6688d.pth'),)
    elif model_name == 'vit':
...
class ImageEncoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.model, self.preprocess = copy.deepcopy(load_from_timm(self.model_name, self.pretrained, args))
        for p in self.model.parameters():
            p.requires_grad = self.trainable

    def forward(self, x):
        if self.model_name == 'clip' and self.pretrained:
            images = self.preprocess(images=x, return_tensors="pt").to(self.device)
            with torch.no_grad():
                output = self.model.get_image_features(**images)   
            return output
...
class CLIPModel_full(nn.Module):
    def __init__(
        ...
        self.text_projection = ProjectionHead(embedding_dim=self.text_embedding, projection_dim=self.image_embedding).to(self.device)
    ...
    def forward(self, image, caption, epoch, similarity=None):
        ...
        image_features = torch.nn.functional.normalize(combined_image_features, p=2, dim=1)
        text_features = torch.nn.functional.normalize(combined_text_features, p=2, dim=1)
